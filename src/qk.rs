...
//! Fixed-order gauss-kronrod quadrature routines.

use num::Float;

fn rescale_error<T>(err: T, res_abs: T, res_asc: T) -> T
where
    T: Float,
{
    let mut scaled_err = err.abs();

    if !res_asc.is_zero() && !scaled_err.is_zero() {
        let scale = (T::from(200).unwrap() * scaled_err / res_asc).powf(T::from(1.5).unwrap());

        scaled_err = if scale < T::one() {
            res_asc * scale
        } else {
            res_asc
        };
    ...
    if res_abs > T::min_positive_value() / (T::from(50).unwrap() * T::epsilon()) {
        let min_err = T::from(50).unwrap() * T::epsilon() * res_abs;
...
pub fn qk<F, T>(func: F, a: T, b: T, xgk: &[T], wgk: &[T], wg: &[T]) -> (T, T, T, T)
where
    T: Float,
    F: Fn(T) -> T,
{
    ...
    let mut fv1: Vec<T> = vec![T::zero(); n - 1];
    let mut fv2: Vec<T> = vec![T::zero(); n - 1];

    let half = T::from(0.5).unwrap();

    let center: T = half * (a + b);
    let half_len: T = half * (b - a);
    let abs_half_len: T = half_len.abs();
    let f_center: T = func(center);

    let mut res_gauss = T::zero();
    ...
    for (j, gaussn) in wg.iter().enumerate().take((n - 1) / 2) {
        ...
        res_abs = res_abs + wgk[jtw] * (fval1.abs() + fval2.abs());
    }
...
pub(crate) const QK15: ([f64; 8], [f64; 8], [f64; 4]) = (
    [
...
pub(crate) const QK21: ([f64; 11], [f64; 11], [f64; 5]) = (
    [
...
pub(crate) const QK31: ([f64; 16], [f64; 16], [f64; 8]) = (
    [
...
pub(crate) const QK41: ([f64; 21], [f64; 21], [f64; 10]) = (
    [
...
pub(crate) const QK51: ([f64; 26], [f64; 26], [f64; 13]) = (
    [
...
pub(crate) const QK61: ([f64; 31], [f64; 31], [f64; 15]) = (
    [
...
mod test {
    ...
    fn test_qk15_smooth_pos() {
        ...
        let (result, abserr, resabs, resasc) = qk(f, 0.0, 1.0, &QK15.0, &QK15.1, &QK15.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 1.0, 0.0, &QK15.0, &QK15.1, &QK15.2);
    ...
    fn test_qk15_oscillating() {
        ...
        let alpha = 1.3;
        let f = |x| f3(x, alpha);
        let (result, abserr, resabs, resasc) = qk(f, 0.3, 2.71, &QK15.0, &QK15.1, &QK15.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 2.71, 0.3, &QK15.0, &QK15.1, &QK15.2);
    ...
    fn test_qk21_smooth_pos() {
        ...
        let (result, abserr, resabs, resasc) = qk(f, 0.0, 1.0, &QK21.0, &QK21.1, &QK21.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 1.0, 0.0, &QK21.0, &QK21.1, &QK21.2);
    ...
    fn test_qk21_oscillating() {
        ...
        let alpha = 1.3;
        let f = |x| f3(x, alpha);
        let (result, abserr, resabs, resasc) = qk(f, 0.3, 2.71, &QK21.0, &QK21.1, &QK21.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 2.71, 0.3, &QK21.0, &QK21.1, &QK21.2);
    ...
    fn test_qk31_smooth_pos() {
        ...
        let (result, abserr, resabs, resasc) = qk(f, 0.0, 1.0, &QK31.0, &QK31.1, &QK31.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 1.0, 0.0, &QK31.0, &QK31.1, &QK31.2);
    ...
    fn test_qk31_oscillating() {
        ...
        let alpha = 1.3;
        let f = |x| f3(x, alpha);
        let (result, abserr, resabs, resasc) = qk(f, 0.3, 2.71, &QK31.0, &QK31.1, &QK31.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 2.71, 0.3, &QK31.0, &QK31.1, &QK31.2);
    ...
    fn test_qk41_smooth_pos() {
        ...
        let (result, abserr, resabs, resasc) = qk(f, 0.0, 1.0, &QK41.0, &QK41.1, &QK41.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 1.0, 0.0, &QK41.0, &QK41.1, &QK41.2);
    ...
    fn test_qk41_oscillating() {
        ...
        let alpha = 1.3;
        let f = |x| f3(x, alpha);
        let (result, abserr, resabs, resasc) = qk(f, 0.3, 2.71, &QK41.0, &QK41.1, &QK41.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 2.71, 0.3, &QK41.0, &QK41.1, &QK41.2);
    ...
    fn test_qk51_smooth_pos() {
        ...
        let (result, abserr, resabs, resasc) = qk(f, 0.0, 1.0, &QK51.0, &QK51.1, &QK51.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 1.0, 0.0, &QK51.0, &QK51.1, &QK51.2);
    ...
    fn test_qk51_oscillating() {
        ...
        let alpha = 1.3;
        let f = |x| f3(x, alpha);
        let (result, abserr, resabs, resasc) = qk(f, 0.3, 2.71, &QK51.0, &QK51.1, &QK51.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 2.71, 0.3, &QK51.0, &QK51.1, &QK51.2);
    ...
    fn test_qk61_smooth_pos() {
        ...
        let (result, abserr, resabs, resasc) = qk(f, 0.0, 1.0, &QK61.0, &QK61.1, &QK61.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 1.0, 0.0, &QK61.0, &QK61.1, &QK61.2);
    ...
    fn test_qk61_oscillating() {
        ...
        let alpha = 1.3;
        let f = |x| f3(x, alpha);
        let (result, abserr, resabs, resasc) = qk(f, 0.3, 2.71, &QK61.0, &QK61.1, &QK61.2);
        ...
        let (result, abserr, resabs, resasc) = qk(f, 2.71, 0.3, &QK61.0, &QK61.1, &QK61.2);
